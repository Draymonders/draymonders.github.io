...
**注意**: 尝试之前会利用 `hasQueuedPredecessors()` 是公平锁加锁时判断等待队列中是否存在`有效节点`的方法。如果返回False，说明当前线程可以争取共享资源；如果返回True，说明队列中存在有效节点。
...
> AQS 中的队列是由 Node 节点组成的双向链表实现的。双向链表中，第一个节点为虚节点，其实并不存储任何信息，只是占位。
...
首先判断队列是否为空，为空时初始化，不为空时则将封装好的 `Node` 利用 `CAS` 写入队尾。
