...
- 采用的是头插法，在`_dictKeyIndex`会去寻找index（没在扩容，只查`ht[0]`，如果扩容还查`ht[1]`）
- 正在扩容，会直接把新entry写入`ht[1]`，否则写入`ht[0]`
...
和插入差不多
- 当前在rehash，需要扫描`ht[0]`和`ht[1]`; 否则只扫描`ht[0]`
...
- `dictRehash`函数是找一个新的bucket，rehash到`ht[1]`里面，还是头插法
...
keys遍历为安全遍历，遍历时候`d->iterators != 0`, 因此不能执行rehash操作
